...
- 게임플레이어
  - 3개의 숫자를 입력한다
  - 잘못된 값을 입력할 경우 예외 발생 후 종료되어야한다.
- 상대방(컴퓨터)
  - 1~9까지, 서로 다른 수, 3자리 숫자를 생성한다
  - 플레이어가 제출한 숫자에 대해서 채점을 한다
- 힌트
  - 스트라이크, 볼, 낫싱
  - 스트라이크를 제외하고 contains하면 볼, 그것도 아니면 낫싱
...
- 3자리의 서로 다른 숫자를 랜덤 생성할 수 있어야한다
- 숫자를 비교할 수 있어야한다
  - 볼, 스트라이크, 낫싱 결과 판단 할 수 있어야한다
- 반복 입력이 가능해야한다
- 게임 반복이 가능해야한다
...
- 유효한 숫자 검증 기능
  - 숫자 확인 기능
  - 3자리 숫자가 아닐 경우 예외 발생
  - 1~9와 다른 입력일 경우 예외 발생
  - 중복된 숫자일 경우 예외 발생
  
컴퓨터
- 정답 생성 기능(랜덤, 서로 다른 숫자)
- 결과 분석 기능
- 힌트 제공 기능
